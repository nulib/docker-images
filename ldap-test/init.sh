...
appSetup () {
	...
	if [[ ! -f /etc/samba/external/smb.conf ]]; then
		...
		if [[ ${JOIN,,} == "true" ]]; then
			...
			samba-tool domain provision --use-ntvfs --use-rfc2307 --domain=${URDOMAIN} --realm=${UDOMAIN} --server-role=dc --dns-backend=SAMBA_INTERNAL --adminpass=${DOMAINPASS} ${HOSTIP_OPTION}
